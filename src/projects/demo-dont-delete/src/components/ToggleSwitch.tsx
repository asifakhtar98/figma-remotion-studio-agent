...
export const ToggleSwitch: FC<ToggleSwitchProps> = ({
  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-[20px] bg-white border border-slate-200/90 shadow-xs">
      ...
      <div
        className={`w-12 h-7 rounded-full p-1 cursor-pointer flex items-center shrink-0 border ${
          enabled
            ? 'bg-indigo-600 border-indigo-600 justify-end shadow-md shadow-indigo-600/30'
            : 'bg-slate-200 border-slate-300 justify-start'
        }`}
      >
        <div className="w-5 h-5 rounded-full bg-white shadow-md transform" />
      </div>
    </div>
  );
};

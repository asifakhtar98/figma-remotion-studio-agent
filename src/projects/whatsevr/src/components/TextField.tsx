...
export const TextField: FC<TextFieldProps> = ({icon, placeholder, value, trailing}) => {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-5 py-3.5 min-h-[52px] shadow-sm">
      <span className="text-gray-400">{icon}</span>
      <span className={`flex-1 text-base ${value ? 'text-gray-900 font-medium' : 'text-gray-400'}`}>
        {value || placeholder}
      </span>
